...
uintptr_t offset_SteamDetect_Awake = 0x0; //SteamDetection$$Awake
...
void photon_hook(void* _appSettings, bool startInOfflineMode, void* mi) {
    if (_appSettings && il2cpp_string_new != nullptr) {
        WriteMem<uintptr_t>((uintptr_t)_appSettings + 0x10, (uintptr_t)il2cpp_string_new("")); //pun2 appid
        WriteMem<uintptr_t>((uintptr_t)_appSettings + 0x28, (uintptr_t)il2cpp_string_new("")); //voice appid
    }
    return o_photon_hook(_appSettings, startInOfflineMode, mi);
}

using o_steam_hook_t = void(*)(void*, void*);
o_steam_hook_t o_steam_hook = nullptr;

void* steam_hook(void* __this, void* mi) {
    return nullptr;
}
...
void Main() {
    ...
    uintptr_t targetAddr2 = GameAssembly + offset_SteamDetect_Awake;

    if (MH_CreateHook((LPVOID)targetAddr, (LPVOID)&photon_hook, (LPVOID*)&o_photon_hook) == MH_OK) {
        MH_EnableHook((LPVOID)targetAddr);
    }
    if (MH_CreateHook((LPVOID)targetAddr2, (LPVOID)&steam_hook, (LPVOID*)&o_steam_hook) == MH_OK) {
